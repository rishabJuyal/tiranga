...
const TermsBox = () => {
  return (
    <div className="bg-[#2b3270] w-full rounded-xl p-3 mb-3 text-white text-sm">
      <div className="w-full flex justify-around items-center">
        <img src={logo} className=" h-10" alt="Tirangagame" />
      ...
      <div className="my-3 flex flex-col gap-5">
        {terms.map((term) => (
          <div key={term.id} className="flex gap-1">
            <Stop color="primary" sx={{rotate: "45deg", marginTop: "5px", fontSize: "13px"}} />
            <p>{term.text}</p>
          </div>
        ))}
      </div>
      <p className="text-red-500">
        Gambling can be addictive, please play rationally. Tiranga only accepts
